...
import {
  OrientationError,
  updateOrientationWatchedSeconds,
...
const handler = async (req, res) => {
  ...
  const { orientationVideoId, watchedSeconds } = req.body || {};
  ...
  try {
    const { video, progress } = await updateOrientationWatchedSeconds(
      session.user.userId,
      orientationVideoId,
      watchedSeconds,
    );
    ...
    return res.status(500).json({
      errors: { error: { msg: 'Failed to update orientation progress' } },
    });
  }
};
