...
const Broadcast = () => {
    ...
    if (
      ...
      if (program.length > 0) {
        filtered = filtered.filter((student) =>
          program.includes(student.program)
        );
      }
      ...
      if (accreditation.length > 0) {
        filtered = filtered.filter((student) =>
          accreditation.includes(student.accreditation)
        );
      }
      ...
      return new Promise((resolve, reject) => {
        uploadTask.on(
          ...
          (error) => {
            ...
            const errorMessage =
              error?.message || error?.code || 'Failed to upload image';
  ...
  const handleSendClick = async () => {
    ...
    try {
      ...
      if (totalAttachmentSize > 15 * 1024 * 1024) {
        toast.error(
          'Total attachment size exceeds 15 MB. Please reduce the size or number of attachments.'
        );
        setIsSending(false);
  ...
  const generatePreviewContent = async () => {
    ...
    try {
      ...
      if (!emailContent || emailContent.trim() === '') {
        toast.error('Please add some content before generating preview.');
      ...
      // Create a temporary DOM parser to handle HTML more reliably
      const parser = new DOMParser();
      const doc = parser.parseFromString(processedContent, 'text/html');

      // Process in document order to maintain content sequence
      const bodyElement = doc.body;
      const allElements = Array.from(bodyElement.children);

      // Create a new container to rebuild the content in correct order
      const newContentContainer = doc.createElement('div');
      ...
      allElements.forEach((element) => {
        if (element.tagName === 'P') {
          ...
          if (images.length > 0) {
            // Create a copy of the paragraph to work with
            const paragraphClone = element.cloneNode(true);
            const imagesInParagraph = paragraphClone.querySelectorAll('img');

            // Process text content first
            const textContent = paragraphClone.innerHTML;
            const hasText = paragraphClone.textContent.trim().length > 0;

            // If there's text content, add the paragraph with padding
            if (hasText) {
              const textParagraph = doc.createElement('p');
              textParagraph.innerHTML = textContent;

              // Remove images from text paragraph
              textParagraph
                .querySelectorAll('img')
                .forEach((img) => img.remove());

              // Only add if there's still content after removing images
              if (
                textParagraph.innerHTML.trim() !== '' ||
                textParagraph.textContent.trim() !== ''
              ) {
                textParagraph.style.margin = '0';
                textParagraph.style.padding = '25px';
                newContentContainer.appendChild(textParagraph);
              }
            }

            // Now add images after the text (in their original positions within the paragraph)
            imagesInParagraph.forEach((img) => {
              ...
              newContentContainer.appendChild(wrapper);
            });

            // Remove the original paragraph
            element.remove();
          } else {
            // Paragraph without images - just add padding
          ...
          // Other elements (div, span, etc.) - preserve them as-is
          newContentContainer.appendChild(element);
        }
      });

      // Process any remaining images inside other elements
      const remainingImages = newContentContainer.querySelectorAll(
        'img:not([style*="display:block"])'
      );
      ...
      let sanitizedEmailContent = newContentContainer.innerHTML;

      // Additional regex cleanup for email clients (fallback)
      // First, ensure paragraph order is preserved
      sanitizedEmailContent = sanitizedEmailContent
        ...
        .replace(/<img([^>]*)>/gi, (match, attributes) => {
          if (!attributes.includes('style=')) {
            return `<img style="display:block;width:100%;height:auto;margin:0;padding:0;border:0;max-width:100%;"${attributes}>`;
          }
          return match;
        });
      ...
      let parentName = 'Sample Parent';
      if (guardianEmails.length > 0) {
        const firstGuardian = guardianEmails[0];
        if (firstGuardian.primaryGuardianName) {
          parentName = firstGuardian.primaryGuardianName.split(' ')[0];
        }
      }
      ...
      setTestEmailSubject(emailSubject || 'Test Email Subject');
      setTestEmailSender(emailSender || '');
      setShowPreview(true);
    } catch (error) {
      console.error('Preview generation error:', error);
      toast.error(
        error.message === 'Generated preview HTML is empty'
          ? 'Preview content is empty. Please check your content.'
  ...
  const sendTestEmail = async () => {
    ...
    if (!hasContent) {
      toast.error(
        'Please add content to the email before sending a test email.'
      );
      return;
    }
    ...
    if (!hasTestSender) {
      toast.error(
        'Please select a test email sender before sending a test email.'
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(
          result.error?.msg || 'Failed to send test email. Please try again.'
        );
      }
    } catch (error) {
                    ...
                    {filterValueOptions.map(({ value, label }) => (
                      <label key={value} className="block">
                        <input
                          ...
                          value={value}
                          onChange={(e) =>
                            handleCheckboxChange(e, setFilterValues)
                          }
                        />
                  ...
                  onClick={() => {
                    if (singularEmail && singularParent) {
                      setGuardianEmails((prev) => [
                        ...
                        {
                          email: singularEmail,
                          primaryGuardianName: singularParent,
                        },
                      ]);
                      setSingularEmail('');
                      setSingularParent('');
              ...
              <div className="mt-4">
                ...
                {guardianEmails.map((guardian, index) => (
                  <div key={index} className="flex justify-between">
                    <span>
                      {guardian.primaryGuardianName} - {guardian.email}
                    </span>
                    <button
                      onClick={() => {
                        setGuardianEmails(
                          guardianEmails.filter((_, i) => i !== index)
                        );
                      }}
        ...
        <div className="w-full max-w-4xl">
          <div className="mb-4 p-2 bg-gray-100 rounded text-sm">
            ...
            <p>
              ...
              {guardianEmails.length === 1 ? 'parent' : 'parents'}
            </p>
            <p className="mt-2 text-xs text-gray-600 italic">
              Note: This is a preview. Actual email may vary slightly in
